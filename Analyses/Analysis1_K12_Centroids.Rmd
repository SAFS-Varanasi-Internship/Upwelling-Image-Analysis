...
```{r}
set.seed(1221966)
out_kmeans <- kmeans(X_norm, n_K, iter.max=25, nstart=100)
```
...
library(tmap)
pal <- topo.colors(100)
pal <- colorRamps::matlab.like2(100)
...
Distance based on correlation. In this case, the mean temperature of the centroid images are not important since correlation is scale invariate. So this distance is based on *shape*. The `dendIMG()` function is shown below.
```{r echo=FALSE}
...
```{r results='asis'}
writeLines(readLines(file.path(here::here(), "R", "dendIMG.R")))
```