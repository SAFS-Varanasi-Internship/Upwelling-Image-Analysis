...
library(dendextend)
dd <- as.dist((1 - cor(t(CC)))/2)
dend <- as.dendrogram(hclust(dd))
...
labels_colors(dend) <- as.character(cols)[order.dendrogram(dend)]
plot(dend)
rect.dendrogram( dend, k=6, which=2, lty = 5, lwd = 0, col=rgb(0.1, 0.2, 0.4, 0.1) ) 
rect.dendrogram( dend, k=6, which=1, lty = 5, lwd = 0, col=rgb(0.1, 0.2, 0.4, 0.1) ) 
```

Removing the mean off the rows seems to capture the upwelling better. Images are shown for each centroid.
```{r echo=FALSE}
par(mar=c(10,3,3,0))
mat <- t(scale(t(out_kmeans$centers), scale=FALSE))
rownames(mat) <- labs
dd <- dist(mat)
dend <- as.dendrogram(hclust(dd))
cols <- cut(apply(CC,1,mean)-mean(CC), breaks=c(-2,-0.4,0.4,2), labels=c("blue", "black", "red"))
labels_colors(dend) <- as.character(cols)[order.dendrogram(dend)]
plot(dend)
rect.dendrogram( dend, k=4, which=4, lty = 5, lwd = 0, col=rgb(0.1, 0.2, 0.4, 0.1) ) 
for(i in 1:12)
rasterImage(as.raster(img.list[[order.dendrogram(dend)[i]]], col=pal), i-0.4, -10, i+0.4, -7, 
            xpd=TRUE, interpolate = FALSE)
```